...
function extractOption(val) {
  if (!val) return val;
  // Try proper parse first
  try {
    const parsed = JSON.parse(val);
    return parsed?.tabledata?.[0]?.Option ?? val;
  } catch {
    // Fall back to regex for truncated values
    const match = val.match(/"Option"\s*:\s*"([^"]+)"/);
...
async function main() {
  ...
  // Can't use resp.json() — the API returns malformed JSON with bare backslashes
  const text = await resp.text();
  const cleaned = text.replace(/\\(?!["\\/bfnrtu])/g, () => {
    console.warn("Warning: fixed invalid escape sequence in API response");
    return "\\\\";
  });
  ...
  records.forEach((r) => {
    r.workEventType = extractOption(r.workEventType);
    r.source = extractOption(r.source);
    r.permitType = extractOption(r.permitType);
  });
  ...
  const pts = records
    ...
    .map((r) => {
      const pt = turf.point([parseFloat(r.longitude), parseFloat(r.latitude)], {
  ...
  if (pts.length === 0) {
    console.warn(
      "WARNING: No points found after filtering. Not updating file.",
    );
    process.exit(1);
  }
  ...
  fs.writeFileSync(
    OUT_PATH,
    JSON.stringify(turf.featureCollection(pts), null, 2),
  );
